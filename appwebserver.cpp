...
#endif
{
  ...
  pServer->on("/cmd/advent/1", []() {
      FireStarter_SetAdvent(1,false);
      pServer->send(200, "text/plain", "accepted");
  });

  pServer->on("/cmd/advent/2", []() {
      FireStarter_SetAdvent(2,false);
      pServer->send(200, "text/plain", "accepted");
  });

  pServer->on("/cmd/advent/3", []() {
      FireStarter_SetAdvent(3,false);
      pServer->send(200, "text/plain", "accepted");
  });

  pServer->on("/cmd/advent/4", []() {
      FireStarter_SetAdvent(4,false);
      pServer->send(200, "text/plain", "accepted");
  });

 pServer->on("/cmd/simulate/1", []() {
      FireStarter_SetAdvent(1,true);
      pServer->send(200, "text/plain", "accepted");
  });

  pServer->on("/cmd/simulate/2", []() {
      FireStarter_SetAdvent(2,true);
      pServer->send(200, "text/plain", "accepted");
  });

  pServer->on("/cmd/simulate/3", []() {
      FireStarter_SetAdvent(3,true);
      pServer->send(200, "text/plain", "accepted");
  });

  pServer->on("/cmd/simulate/4", []() {
      FireStarter_SetAdvent(4,true);
      pServer->send(200, "text/plain", "accepted");
  });
